...
public class LoginServiceImpl extends BaseService implements LoginService {
    ...
    public ResponseBase login(String username, String password, HttpSession session, HttpServletResponse response) throws Exception {
        Map<String, Object> data = new HashMap<>();
        LoginCheckResponseDto check = userRepository.getFirstByUsername(username);
        ...
        if (check.getUserAccount().getLockoutEndTime() != null && check.getUserAccount().getLockoutEndTime().isAfter(Instant.now())) {
            setValueForHeaderFooter(data, true, false, false, true);

            // nếu đúng mật khẩu
            if (check.getUserAccount().getPassword().equals(userUtil.hashPassword(password))) {
                int numberMinutes = check.getUserAccount().getLockoutEndTime().atZone(ZoneId.systemDefault()).getMinute() - Instant.now().atZone(ZoneId.systemDefault()).getMinute();
                String strMinutes = check.getUserAccount().getLockoutEndTime().atZone(ZoneId.systemDefault()).getMinute() - Instant.now().atZone(ZoneId.systemDefault()).getMinute() == 1 ? "minute" : "minutes";

                data.put("error", String.format("Your account is locked. Please come back after %d %s", numberMinutes, strMinutes));
                return new ResponseBase().withData(data).withViewName("login");
            }

            userAccountRepository.setLockoutEndTime(Instant.now().plusSeconds(5 * 60), check.getUserAccount().getUserAccountId());
        ...
        if (!check.getUserAccount().getPassword().equals(userUtil.hashPassword(password))) {
            ...
            int userFailedLoginCount = check.getUserAccount().getFailedLoginCount() + 1;
            userAccountRepository.setLockoutEndTimeAndFailedLoginCount(Instant.now().plusSeconds(10 * 60), userFailedLoginCount, check.getUserAccount().getUserAccountId());
        ...
        userAccountRepository.setFailedLoginCount(0, check.getUserAccount().getUserAccountId());

        UserProfileResponseDto userDto = userMapper.toUserProfileResponseDto(check);
        session.setAttribute("user", userDto);

        Cookie cookie = new Cookie("userId", check.getUser().getUserId().toString());
